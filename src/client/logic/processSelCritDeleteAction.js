...
export default createNamedLogic('processSelCritDeleteAction', {
  ...
  type: String(actions.selCrit.delete),
  ...
  process({getState, action, log, api}, dispatch) {
    ...
    if (inMemoryOnly) {
      log.debug(()=>`in-memory deletion of selCrit: ${selCrit.name} by emitting: actions.selCrit.delete.complete `);
      dispatch( actions.selCrit.delete.complete(selCrit, impactView) );
    }
    ...
    // for persistent DB selCrit deletion, issue issue the async API delete request
      ...
      api.filters.deleteFilter(selCrit, log)
         ...
         .catch( err => {
           ...
           dispatch( actions.selCrit.delete.fail(selCrit,
