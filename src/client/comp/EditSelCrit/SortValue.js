...
@ReactRedux.connect( (appState, ownProps) => {
  return {
    selectedSortOptions: selectors.getEditSelCrit(appState).extra.selectedSortOptions,
  }
})
...
export default class SortValue extends React.Component {
  ...
  handleAscDecToggle(sortOption) {
    ...
    p.dispatch( actions.selCrit.edit.change.sort(selectedSortOptions) );
  }
  ...
  handleReposition(moveIndex,    // move selected options [moveIndex]
    ...
    p.dispatch( actions.selCrit.edit.change.sort(newOpts) );
  }
